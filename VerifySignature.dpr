...
uses
  ...
  VerifyController in 'VerifyController.pas',
  WebModule in 'WebModule.pas' {VerifyWebModule: TWebModule};
...
begin
  ...
  try
    ...
    dotEnvConfigure(
      function: IMVCDotEnv
      begin
        Result := NewDotEnv.UseStrategy(TMVCDotEnvPriority.FileThenEnv)                                       //if available, by default, loads default environment (.env)
          ...
          .UseLogger(
          procedure(LogItem: string)
          begin
            LogW('dotEnv: ' + LogItem);
          end).Build();             //uses the executable folder to look for .env* files
      end);
